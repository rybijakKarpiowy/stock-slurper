...
function App() {
		...
		if (fromInput) {
			if (!reqData.from) {
				...
				toInput.min = new Date(
					new Date(reqData.from).setDate(
						new Date(reqData.from).getDate() + 1
					)
				)
					.toISOString()
					.split("T")[0];
		...
		if (toInput) {
			if (!reqData.to) {
				...
				fromInput.max = new Date(
					new Date(reqData.to).setDate(
						new Date(reqData.to).getDate() - 1
					)
				)
					.toISOString()
					.split("T")[0];
	...
	const handleSubmit = async (
		...
		const socket = new WebSocket(
			"wss://stock-slurper-production.up.railway.app"
		);
		...
		socket.onmessage = (event) => {
			const { message, spreadsheetLink, progress } = JSON.parse(
				event.data
			);
			if (message) {
				if (
		...
		<div className="main">
			...
			{company ? (
				...
				>
						...
						>
							...
							>
								...
								<input
									...
									onChange={(e) =>
										setReqData({
											days: (e.target as HTMLInputElement)
												.value,
											from: "",
											to: "",
										})
							...
							>
								...
								<input
									...
									onChange={(e) =>
										setReqData({
											days: "",
											from: (e.target as HTMLInputElement)
												.value,
											to: reqData.to,
										})
									}
								...
								<input
									...
									onChange={(e) =>
										setReqData({
											days: "",
											from: reqData.from,
											to: (e.target as HTMLInputElement)
												.value,
										})
									}
									disabled={loading}
					...
					<button
						className={`submit button ${
							((!reqData.days &&
								(!reqData.from || !reqData.to)) ||
								loading) &&
							"disabled"
						} ${loading && "loadingBorder"}`}
						type="submit"
						disabled={
							(!reqData.days && (!reqData.from || !reqData.to)) ||
							loading
						}
					>
				...
				<>
					<h1>Lokomotywy</h1>
					<span>
						Wybierz firmę, której dane chcesz przeanalizować
					</span>
